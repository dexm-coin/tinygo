...
func (fr *frame) evalBasicBlock(bb, incoming llvm.BasicBlock) (retval Value, outgoing []llvm.Value, err error) {
	for inst := bb.FirstInstruction(); !inst.IsNil(); inst = llvm.NextInstruction(inst) {
		if fr.Debug {
			inst.Dump()
			println()
		}
		if _, ok := fr.locals[inst]; ok {
			// already handled (e.g. runtime.alloc + bitcast)
			continue
		}
		switch {
		...
		case !inst.IsALoadInst().IsNil():
			...
			if value.Type() != inst.Type() {
				panic("initinterp: load: type does not match")
			}
		...
		case !inst.IsAGetElementPtrInst().IsNil():
			...
			if result.Type() != inst.Type() {
				...
				panic("initinterp: gep: type does not match")
			}
		...
		case !inst.IsABitCastInst().IsNil() && inst.Type().TypeKind() == llvm.PointerTypeKind:
			value := fr.getLocal(inst.Operand(0))
			fr.locals[inst] = &PointerCastValue{Underlying: value, CastType: inst.Type()}
		...
		case !inst.IsACallInst().IsNil():
			...
			case !callee.IsAFunction().IsNil():
				...
				ret, err := fr.Function(callee, params, fr.pkgName)
				if err != nil {
					return nil, nil, err
				}
				if inst.Type().TypeKind() != llvm.VoidTypeKind {
		...
		case !inst.IsABranchInst().IsNil() && inst.OperandsCount() == 3:
			...
			if !cond.IsConstant() {
				return nil, nil, errors.New("initinterp: branch on a non-constant")
			} else {
	...
	panic("initinterp: reached end of basic block without terminator")
}

...
func (v *LocalValue) Load() llvm.Value {
	...
	default:
		panic("initinterp: load from a constant")
	}
}
...
func (v *LocalValue) Store(value llvm.Value) {
	...
	case llvm.GetElementPtr:
		...
		return
	default:
		panic("initinterp: store on a constant")
	}
}
...
func (v *LocalValue) GetElementPtr(indices []uint32) Value {
	...
	default:
		panic("initinterp: GEP on a constant")
	}
}

func (v *LocalValue) String() string {
	return "&LocalValue{Type: " + v.Type().String() + "}"
}
...
func (v *AllocaValue) Store(value llvm.Value) {
	if v.Underlying.Type() != value.Type() {
		panic("initinterp: trying to store to an alloca with a different type")
	}
...
func (v *GetElementPtrValue) Value() llvm.Value {
	if v.Alloca.Dirty {
		...
		panic("initinterp: todo: pointer to alloca gep")
	}
}
...
func (v *GetElementPtrValue) GetElementPtr(indices []uint32) Value {
	if v.Alloca.Dirty {
		panic("initinterp: todo: gep on a dirty gep")
	} else {
...
// Load panics: it is not (yet) possible to load directly from a bitcast.
func (v *PointerCastValue) Load() llvm.Value {
	panic("initinterp: load from a pointer bitcast")
}

// Store panics: it is not (yet) possible to store directly to a bitcast.
func (v *PointerCastValue) Store(value llvm.Value) {
	panic("initinterp: store on a pointer bitcast")
}
...
func (v *PointerCastValue) GetElementPtr(indices []uint32) Value {
	panic("initinterp: GEP on a pointer bitcast")
}
...
func (v *MapValue) Value() llvm.Value {
	...
	for i, key := range v.Keys {
		...
		if key.Type().TypeKind() == llvm.StructTypeKind && key.Type().StructName() == "runtime._string" {
			...
			panic("initinterp: map key type not implemented: " + key.Type().String())
		}
		hash := v.hash(keyBuf)
	...
	case *PointerCastValue:
		...
		if v.ValueType.IsNil() {
			...
			if int(v.Eval.TargetData.TypeAllocSize(v.ValueType)) != v.ValueSize {
				panic("initinterp: map store value type has the wrong size")
			}
		} else {
			if value.Type() != v.ValueType {
				panic("initinterp: map store value type is inconsistent")
			}
		}
	default:
		panic("initinterp: todo: handle map value pointer")
	}
